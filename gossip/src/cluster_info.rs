...
impl ClusterInfo {
    ...
    /*pub fn staked_tpu_quic_addresses(&self, stakes: Arc<HashMap<Pubkey, u64>>, total_stake: u64) -> Vec<(Pubkey, SocketAddr)> {
        let self_pubkey = self.id();
        let gossip_crds = self.gossip.crds.read().unwrap();
        gossip_crds
            .get_nodes_contact_info()
            .filter(|node| {
                node.pubkey() != &self_pubkey
                    && stakes.get(&node.pubkey())
                        .map_or(false, |&stake| is_staked_node(stake, total_stake))
