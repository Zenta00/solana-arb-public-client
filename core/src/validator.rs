...
use {
    ...
    arbitrage::arbitrage_streamer::ArbitrageStreamer,
...
pub struct Validator {
    arbitrage_streamer: Option<ArbitrageStreamer>,
...
impl Validator {
    ...
    pub fn new(
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::CleaningAccounts;
        ...
        let (accounts_update_sender, accounts_update_receiver) = unbounded();


        let tracked_pubkeys = Arc::new(RwLock::new(HashSet::new()));

        let accounts_update_notifier = Some(
            Arc::new(
                AccountsUpdateNotifierImpl::new(
                    tracked_pubkeys.clone(),
        ...
        let accounts_update_notifier = accounts_update_notifier.map(|notifier| {
            notifier as Arc<dyn AccountsUpdateNotifierInterface + Send + Sync>
        });

        let (slot_status_sender, slot_status_receiver) = unbounded();

        let slot_status_notifier = Some(
            Arc::new(
                RwLock::new(
                    SlotStatusNotifierImpl::new(
                        slot_status_sender,
                    )
                )
            )
        ...
        let slot_status_notifier = slot_status_notifier.map(|notifier| {
            notifier as Arc<RwLock<dyn SlotStatusNotifierInterface + Send + Sync>>
        });
        ...
        let (
            ...
            TransactionHistoryServices {
                transaction_status_sender: _transaction_status_sender,
        ...
        let (tx_status_sender, tx_status_receiver) = unbounded();
        let transaction_status_sender = Some(TransactionStatusSender {
            sender: tx_status_sender,
        });
        ...
        *admin_rpc_service_post_init.write().unwrap() = Some(AdminRpcRequestMetadataPostInit {
        ...
        let arbitrage_streamer = ArbitrageStreamer::new(
            accounts_update_receiver,
            tx_status_receiver,
        ...
        /* 
        let arbitrage_service = ArbitrageService::new(
            Arc::new(request_processor_clone.unwrap()),
            tx_status_receiver,
            Arc::new(RwLock::new(leader_info.unwrap())),
            accounts_update_receiver,
            tracked_pubkeys,
            exit,
        );
        */
        Ok(Self {
            arbitrage_streamer: Some(arbitrage_streamer),
